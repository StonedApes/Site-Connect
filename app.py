...
def optimize_resources(company_id):
    # Fetch data
    sites = Site.query.filter_by(company_id=company_id).all()
    projects = Project.query.filter_by(company_id=company_id).all()
    equipment = Equipment.query.filter_by(company_id=company_id).all()
    employees = Employee.query.filter_by(company_id=company_id).all()
    inventory = Inventory.query.filter_by(company_id=company_id).all()

    prob = LpProblem("Resource_Optimization", LpMinimize)
    equipment_vars = {
        (e.equipment_id, s.site_id): LpVariable(f"equip_{e.equipment_id}_{s.site_id}", 0, 1, cat='Binary')
        for e in equipment for s in sites
    }
    employee_vars = {
        (e.employee_id, s.site_id): LpVariable(f"emp_{e.employee_id}_{s.site_id}", 0, 1, cat='Binary')
        for e in employees for s in sites
    }
    inventory_vars = {
        (i.inventory_id, s.site_id): LpVariable(f"inv_{i.inventory_id}_{s.site_id}", 0, i.quantity, cat='Integer')
        for i in inventory for s in sites
    }

    # Objective: Minimize total unallocated resources
    prob += lpSum([1 - equipment_vars[(e.equipment_id, s.site_id)] for e in equipment for s in sites]) + \
            lpSum([1 - employee_vars[(e.employee_id, s.site_id)] for e in employees for s in sites]) + \
            lpSum([i.quantity - inventory_vars[(i.inventory_id, s.site_id)] for i in inventory for s in sites])

    # Constraints
    # Each equipment assigned to at most one site
    for e in equipment:
        prob += lpSum([equipment_vars[(e.equipment_id, s.site_id)] for s in sites]) <= 1

    # Each employee assigned to at most one site
    for e in employees:
        prob += lpSum([employee_vars[(e.employee_id, s.site_id)] for s in sites]) <= 1

    # Inventory allocation respects available quantity
    for i in inventory:
        prob += lpSum([inventory_vars[(i.inventory_id, s.site_id)] for s in sites]) <= i.quantity

    # Project requirements (simplified: ensure minimum resources per site with active projects)
    for s in sites:
        active_projects = [p for p in projects if p.site_id == s.site_id and p.status != 'Completed']
        if active_projects:
            prob += lpSum([employee_vars[(e.employee_id, s.site_id)] for e in employees]) >= len(active_projects) * 5  # Min 5 workers per project
            prob += lpSum([equipment_vars[(e.equipment_id, s.site_id)] for e in equipment]) >= len(active_projects)  # Min 1 equipment per project
    ...
    allocations = []
    for s in sites:
        equip_list = [e.name for e in equipment if equipment_vars[(e.equipment_id, s.site_id)].varValue > 0.5]
        emp_list = [e.name for e in employees if employee_vars[(e.employee_id, s.site_id)].varValue > 0.5]
    ...
    return allocations, LpStatus[prob.status]
...
@app.route('/apply_allocation/<int:site_id>', methods=['POST'])
@login_required
def apply_allocation(site_id):
    alloc_data = json.loads(request.form.get('alloc_data', '{}'))
    # Update database (e.g., Equipment.site_id, Employee assignments, Inventory.site_id)
    # This is a placeholder; implement actual updates based on your schema
    flash('Resources reallocated successfully!', 'success')
...
def dashboard():
    try:
        ...
        ).filter_by(company_id=current_user.company_id).first() or [0, 0, 0, 0]
        ...
        ).filter_by(company_id=current_user.company_id).first() or [0, 0, 0]
        ...
        task_progress = db.session.query(
            ...
            db.func.sum(db.case((Task.status != 'Completed', 1), else_=0))
        ).filter_by(company_id=current_user.company_id).first() or [0, 0]
        ...
        resource_util = {'labor': 80, 'equipment': 60, 'materials': 75}
        safety_compliance = {'rate': 90, 'overdue': 2}
        equipment_status = {'available': 5, 'in_use': 3, 'maintenance': 1}
        financials = {'budget': 100000, 'actual': 85000}
        incident_trends = [10, 15, 12, 8, 5]
        site_locations = [{'lat': 12.97, 'lng': 77.59, 'status': 'active'}]
        resource_forecast = {'labor': 90, 'equipment': 70, 'materials': 80}

        logger.info(f"Dashboard data: status_counts={status_counts}, incident_types={incident_types}, task_progress={task_progress}, "
                    f"recent_orders={recent_orders}, projects={projects}, weather_data={weather_data}, compliance_alerts={compliance_alerts}, "
                    f"resource_util={resource_util}, safety_compliance={safety_compliance}, equipment_status={equipment_status}, "
                    f"financials={financials}, incident_trends={incident_trends}, site_locations={site_locations}, "
                    f"resource_forecast={resource_forecast}")
        return render_template('dashboard.html', status_counts=status_counts, incident_types=incident_types,
                              ...
                              resource_util=resource_util, safety_compliance=safety_compliance,
                              equipment_status=equipment_status, financials=financials,
                              incident_trends=incident_trends, site_locations=site_locations,
...
def insights():
    try:
        ...
        # Query for incident counts by type
        ...
        # Query for incident severity counts
        ...
        # Query for subcontractor status counts
        ...
        logger.info("Insights data retrieved successfully")
        return render_template(
            ...
            subcontractor_status_counts=json.dumps(subcontractor_status_counts),
            manpower_dates=json.dumps(manpower_dates),
            manpower_values=json.dumps(manpower_values),
            induction_compliance_rate=induction_compliance_rate,
            license_compliance_rate=license_compliance_rate,
            notifications_count=get_notifications_count(current_user.company_id)
        )
    except Exception as e:
        logger.error(f"Insights error: {str(e)}", exc_info=True)
        flash('Error loading insights. Please try again.', 'danger')
        return render_template('insights.html', status_counts=json.dumps([0, 0, 0, 0]), incident_counts=json.dumps([0, 0, 0]), vehicles_on_road=0, vehicles_in_yard=0, order_dates=json.dumps([]), order_counts=json.dumps([]), severity_counts=json.dumps([0, 0, 0]), subcontractor_status_counts=json.dumps([0, 0, 0]), manpower_dates=json.dumps([]), manpower_values=json.dumps([]), induction_compliance_rate=0, license_compliance_rate=0, notifications_count=get_notifications_count(current_user.company_id)), 500
...
def resource_allocation():
    def optimize_resources(company_id):
        try:
            ...
            if not sites or not projects:
                return [], "No sites or projects found"

            # Initialize problem
            prob = LpProblem("Resource_Optimization", LpMinimize)

            # Variables: Allocate equipment, employees, and inventory to sites
            equipment_vars = {
                (e.equipment_id, s.site_id): LpVariable(f"equip_{e.equipment_id}_{s.site_id}", 0, 1, cat='Binary')
                for e in equipment for s in sites
            }
            ...
            # Objective: Minimize unallocated resources
            prob += lpSum([1 - equipment_vars.get((e.equipment_id, s.site_id), 0) for e in equipment for s in sites]) + \
                    lpSum([1 - employee_vars.get((e.employee_id, s.site_id), 0) for e in employees for s in sites]) + \
                    lpSum([i.quantity - inventory_vars.get((i.inventory_id, s.site_id), 0) for i in inventory for s in sites])

            # Constraints
            # Each equipment assigned to at most one site
            for e in equipment:
                prob += lpSum([equipment_vars.get((e.equipment_id, s.site_id), 0) for s in sites]) <= 1

            # Each employee assigned to at most one site
            for e in employees:
                prob += lpSum([employee_vars.get((e.employee_id, s.site_id), 0) for s in sites]) <= 1

            # Inventory allocation respects available quantity
            for i in inventory:
                prob += lpSum([inventory_vars.get((i.inventory_id, s.site_id), 0) for s in sites]) <= i.quantity

            # Project requirements: Minimum resources per site with active projects
            for s in sites:
                active_projects = [p for p in projects if p.site_id == s.site_id and p.status != 'Completed']
                if active_projects:
                    prob += lpSum([employee_vars.get((e.employee_id, s.site_id), 0) for e in employees]) >= len(active_projects) * 5  # Min 5 workers
                    prob += lpSum([equipment_vars.get((e.equipment_id, s.site_id), 0) for e in equipment]) >= len(active_projects)  # Min 1 equipment

            # Solve
            prob.solve()
            logger.info(f"Optimization solved with status: {LpStatus[prob.status]}")

            # Extract results
            allocations = []
            for s in sites:
                equip_list = [e.name for e in equipment if equipment_vars.get((e.equipment_id, s.site_id), 0).varValue > 0.5]
                emp_list = [e.name for e in employees if employee_vars.get((e.employee_id, s.site_id), 0).varValue > 0.5]
                inv_list = [(i.item_name, int(inventory_vars.get((i.inventory_id, s.site_id), 0).varValue)) for i in inventory if inventory_vars.get((i.inventory_id, s.site_id), 0).varValue > 0]
                allocations.append({
                    'site': s.name,
                    'site_id': s.site_id,
                    'equipment': ', '.join(equip_list) or 'None',
    ...
    try:
        allocations, status = optimize_resources(current_user.company_id)
        logger.info(f"Resource allocation completed with status: {status}")
        return render_template('resource_allocation.html', allocations=allocations, status=status, notifications_count=get_notifications_count(current_user.company_id))
    except ImportError as ie:
        logger.warning(f"PuLP not found, using mock data: {str(ie)}")
        allocations = [
            {'site': 'Site A', 'site_id': 1, 'equipment': '3 Cranes, 2 Forklifts', 'labor': '20 Workers', 'materials': '500 Concrete Blocks, 200 Steel Beams'},
            {'site': 'Site B', 'site_id': 2, 'equipment': '2 Cranes, 1 Excavator', 'labor': '15 Workers', 'materials': '300 Concrete Blocks, 150 Steel Beams'},
            {'site': 'Site C', 'site_id': 3, 'equipment': '1 Crane, 2 Forklifts', 'labor': '25 Workers', 'materials': '400 Concrete Blocks, 100 Steel Beams'}
        ]
        status = "Mock"
        return render_template('resource_allocation.html', allocations=allocations, status=status, notifications_count=get_notifications_count(current_user.company_id))
    except Exception as e:
        logger.error(f"Resource allocation route error: {str(e)}", exc_info=True)
        flash('Error loading resource allocation. Please try again.', 'danger')
        return render_template('resource_allocation.html', allocations=[], status="Error", notifications_count=get_notifications_count(current_user.company_id)), 500
